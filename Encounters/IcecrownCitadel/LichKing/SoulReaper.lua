-- Guardpoint Lich King Soul Reaper data
...
local SoulReaper = {
    ...
    ValidActions={core=true,remaining_core=true,trinket=true,ibf=true,ams=true,army=true,pain=true,sac=true},
}
...
function SoulReaper.IsActionValid(action) return type(action)=="string" and SoulReaper.ValidActions[action]==true end
...
function SoulReaper.GetActions(phase,number)
    ...
    if not plan or not validActionList(plan.before) or not validActionList(plan.after) then return nil end
...
_G.GuardpointLichKing=NS.LichKing
