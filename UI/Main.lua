...
function UI.create()
    ...
    local f=CreateFrame("Frame","GP_Main",UIParent);UI.frame=f;f:SetWidth(190);f:SetHeight(76);f:SetFrameStrata("HIGH")
    f:SetMovable(true);f:EnableMouse(true);f:RegisterForDrag("LeftButton")
    f:SetScript("OnDragStart",function(self)if not S.DB.locked then self:StartMoving() end end)
    f:SetScript("OnDragStop",function(self)self:StopMovingOrSizing();local p,_,rp,x,y=self:GetPoint();S.DB.point=p;S.DB.relPoint=rp;S.DB.x=x;S.DB.y=y end)
...
local function updateButtonState()
    ...
    local f=UI.frame;local t=U.now();local lead=(NS.LichKing and NS.LichKing.Timing and NS.LichKing.Timing.GlowLead) or 5.0;local preActive=(not S.active and S.nextAt and (S.nextAt-t)<=lead and (S.nextAt-t)>0);local postActive=S.active
