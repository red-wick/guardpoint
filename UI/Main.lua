...
function UI.create()
    ...
    local function button()
        ...
        b.glowPixels={}
        local glowPoints={{-15,-18},{-7,-18},{1,-18},{9,-18},{15,-18},{18,-14},{18,-6},{18,2},{18,10},{18,15},{15,18},{7,18},{-1,18},{-9,18},{-15,18},{-18,15},{-18,7},{-18,-1},{-18,-9},{-18,-15}}
        for i=1,#glowPoints do
            local tex=b:CreateTexture(nil,"OVERLAY"); tex:SetTexture("Interface\\Buttons\\WHITE8X8"); tex:SetBlendMode("ADD"); tex:SetWidth(3); tex:SetHeight(3); tex:SetPoint("CENTER",b,"CENTER",glowPoints[i][1],glowPoints[i][2]); tex:SetVertexColor(0.10,1.0,0.20,1); tex:SetAlpha(0); tex:Hide(); b.glowPixels[i]=tex
        end
        ...
        b.used=b:CreateTexture(nil,"OVERLAY"); b.used:SetTexture("Interface\\RaidFrame\\ReadyCheck-Ready"); b.used:SetWidth(22); b.used:SetHeight(22); b.used:SetPoint("CENTER",b,"CENTER",0,0); b.used:SetVertexColor(0.15,1.0,0.15,1); b.used:SetAlpha(1); b.used:Hide()
        b.wasReady=false; b.lastCD=0; b:Hide(); return b
    end
...
function UI.placeButtons(list,buttons,cx)
    local n=math.min(2,#list); for i=1,2 do buttons[i]:Hide() end
    if n==0 then return end
    if n==1 then
        buttons[1]:ClearAllPoints(); buttons[1]:SetPoint("CENTER",UI.frame,"CENTER",cx,-31); buttons[1].data=list[1]; buttons[1].icon:SetTexture(UI.iconFor(list[1])); buttons[1]:Show()
    else
        buttons[1]:ClearAllPoints(); buttons[2]:ClearAllPoints(); buttons[1]:SetPoint("CENTER",UI.frame,"CENTER",cx-22,-31); buttons[2]:SetPoint("CENTER",UI.frame,"CENTER",cx+22,-31)
        for i=1,2 do buttons[i].data=list[i]; buttons[i].icon:SetTexture(UI.iconFor(list[i])); buttons[i]:Show() end
    end
end

function UI.layout()
    local f=UI.frame; local b=f.planBefore or {}; local a=f.planAfter or {}
    f.arrow:Hide(); for i=1,2 do f.before[i]:Hide(); f.after[i]:Hide() end
    if #b==1 and #a==1 and U.same(b[1],a[1]) then
        local btn=f.before[1]; btn:ClearAllPoints(); btn:SetPoint("CENTER",f,"CENTER",0,-31); btn.data=b[1]; btn.icon:SetTexture(UI.iconFor(b[1])); btn:Show(); return
    end
    if #b>0 and #a>0 then
        local beforeCenter=-51; local afterCenter=51; UI.placeButtons(b,f.before,beforeCenter); UI.placeButtons(a,f.after,afterCenter)
        local nearestBefore=beforeCenter+(#b==2 and 22 or 0); local nearestAfter=afterCenter-(#a==2 and 22 or 0); local arrowX=(nearestBefore+nearestAfter)/2
        f.arrow:ClearAllPoints(); f.arrow:SetPoint("CENTER",f,"CENTER",arrowX,-31); f.arrow:Show()
...
local function updateButtonState()
    ...
    local f=UI.frame; local t=U.now(); local LK=NS.EncounterRegistry and NS.EncounterRegistry:GetActive(); local lead=(LK and LK.Timing.GlowLead) or 5.0
    ...
    local function updateOne(b,side)
        local a=b.data
        if not a then for i=1,#b.glowPixels do b.glowPixels[i]:SetAlpha(0); b.glowPixels[i]:Hide() end; b.used:Hide(); b.wasReady=false; return end
        ...
        if used then b.icon:SetVertexColor(0.55,0.55,0.55,1); b.used:Show() else b.icon:SetVertexColor(1,1,1,1); b.used:Hide() end
    ...
    if #((f.planBefore) or {})==1 and #((f.planAfter) or {})==1 and U.same(f.planBefore[1],f.planAfter[1]) then
        updateOne(f.before[1],S.active and 2 or 1); for i=1,#f.after[1].glowPixels do f.after[1].glowPixels[i]:SetAlpha(0); f.after[1].glowPixels[i]:Hide() end; f.after[1].data=nil; return
    end
...
function UI.render()
    ...
    if not S.plan then f:Hide(); return end
    f.planBefore=S.plan.before or {}; f.planAfter=S.plan.after or {}; UI.layout()
