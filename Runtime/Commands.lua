...
local function timing()
    local encounter=NS.EncounterRegistry and NS.EncounterRegistry:Resolve()
    return (encounter and encounter.Timing) or NS.LichKing.Timing or {}
end

function X.RegisterCommands()
    ...
    SlashCmdList["GUARDPOINT"]=function(msg)
        ...
        elseif msg=="test" then
            ...
            S.expire=U.now()+(timing().ReaperDuration or 5.1);S.nextAt=nil;S.used={}
        ...
        elseif msg=="pre" then
            S.test=false;S.active=false;S.reaper=0;S.nextNumber=1;S.nextAt=U.now()+(timing().Prewarn or 8.0);S.plan=nil;if UI.frame then UI.frame:Hide() end
        ...
        elseif msg=="hide" then if UI.frame then UI.frame:Hide() end
