...
SlashCmdList["GUARDPOINT"]=function(msg)
    msg=U.lower(msg)
    local LK=NS.EncounterRegistry and NS.EncounterRegistry:GetActive()
    ...
    elseif msg=="test" then
        if not LK then DEFAULT_CHAT_FRAME:AddMessage("|cff66ccffGP|r: encounter module not selected") return end
        S.test=true;S.encounter=false;S.active=true;S.phase=2;S.reaper=S.reaper+1;if S.reaper>8 then S.reaper=1 end
        S.expire=U.now()+(LK.Timing.ReaperDuration or 5.1);S.nextAt=nil;S.used={}
    ...
    elseif msg=="pre" then
        if not LK then DEFAULT_CHAT_FRAME:AddMessage("|cff66ccffGP|r: encounter module not selected") return end
        S.test=false;S.active=false;S.reaper=0;S.nextNumber=1;S.nextAt=U.now()+(LK.Timing.Prewarn or 8.0);S.plan=nil;UI.frame:Hide()
