...
local function markSpellUsed(id)
    ...
    for _,name in ipairs({"before","after"}) do local list=S.plan[name];for i=1,#list do local a=list[i];if a.kind=="spell" and a.id==id then S.used[U.key(a)]=true end end end
end
local function markItemUsed(slot)
    if not S.plan or not slot then return end
    for _,name in ipairs({"before","after"}) do local list=S.plan[name];for i=1,#list do local a=list[i];if a.kind=="item" and a.slot==slot then S.used[U.key(a)]=true end end end
end
