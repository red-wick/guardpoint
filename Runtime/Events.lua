...
local function scheduleNext() local delay;if S.reaper==0 then delay=(S.phase==3 and P3_FIRST or P2_FIRST) else delay=NEXT_REAPER end;S.nextNumber=S.reaper+1;if S.nextNumber>8 then S.nextNumber=1 end;S.nextAt=U.now()+delay end
local function saveCorePair(pair,number) if not pair or not number then return end;if (S.phase==2 and number==3) or (S.phase==3 and number==1) then S.corePair={};for i=1,#pair do S.corePair[#S.corePair+1]=U.copy(pair[i]) end end end
...
local function phaseReset(p) S.phase=p;S.reaper=0;S.nextAt=nil;S.nextNumber=1;S.active=false;S.expire=0;S.plan=nil;S.used={};S.corePair=nil;S.test=false;scheduleNext();if UI.frame then UI.frame:Hide() end end
local function markSpellUsed(id) if not S.plan then return end;for _,name in ipairs({"before","after"}) do local list=S.plan[name];for i=1,#list do local a=list[i];if a.kind=="spell" and a.id==id then S.used[U.key(a)]=true end end end end
local function markItemUsed(slot) if not S.plan or not slot then return end;for _,name in ipairs({"before","after"}) do local list=S.plan[name];for i=1,#list do local a=list[i];if a.kind=="item" and a.slot==slot then S.used[U.key(a)]=true end end end end
...
local function stopEncounter() S.encounter=false;S.encounterGUID=nil;S.encounterStart=0;S.active=false;S.expire=0;S.nextAt=nil;S.nextNumber=1;S.reaper=0;S.plan=nil;S.used={};S.corePair=nil;S.test=false;if UI.frame then UI.frame:Hide() end end
local function bossStillPresent() if LK and LK.IsUnitPresent(S.encounterGUID) then return true end;return findLKUnit()~=nil end
